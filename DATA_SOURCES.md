...
- **No player-level xG / shot rates** for World Cup squads on free tiers (Understat/FBref
  cover club leagues only; FBref's advanced stats shut down Jan 2026).
...
| **FotMob** (unofficial) | xG, shotmaps | — | **Blocked** — API gated behind a rotating signed header. Not usable. |
...
3. **Consider Sportmonks (paid)** only if we want a true xG model rather than the current
   shot-proxy. Highest quality, but a recurring cost.
